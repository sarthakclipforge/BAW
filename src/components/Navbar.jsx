...
export default function Navbar() {
    ...
    return (
        <>
            <nav ref={navRef} className="fixed top-5 left-1/2 -translate-x-1/2 z-50 bg-white/80 backdrop-blur-2xl border border-white/40 shadow-[0_8px_32px_-4px_rgba(0,0,0,0.08)] rounded-full px-3 py-2 w-[96%] max-w-[1100px] transition-all duration-500 will-change-transform font-sans">
                <div className="w-full px-2">
                    <div className="flex justify-between items-center h-[46px]">
                        <Link to="/" className="flex-shrink-0 flex items-center gap-2 origin-left">
                        ...
                        <button
                            className="md:hidden flex flex-col items-center justify-center w-10 h-10 rounded-full bg-gray-100 hover:bg-gray-200 transition-colors"
                            onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
                            aria-label="Toggle mobile menu"
                        >
                            <span className={`block w-4.5 h-0.5 bg-primary rounded-full transition-all duration-300 ${isMobileMenuOpen ? 'rotate-45 translate-y-[3px] w-5' : 'w-5'}`}></span>
                            <span className={`block h-0.5 bg-primary rounded-full transition-all duration-300 mt-1 ${isMobileMenuOpen ? 'opacity-0 w-5' : 'w-3.5'}`}></span>
                            <span className={`block w-4.5 h-0.5 bg-primary rounded-full transition-all duration-300 mt-1 ${isMobileMenuOpen ? '-rotate-45 -translate-y-[5px] w-5' : 'w-5'}`}></span>
                        </button>
                    </div>
                </div>
            </nav>
            ...
            {/* Mobile Menu Panel */}
            <div
                className={`fixed top-0 right-0 h-full w-[85%] max-w-[380px] bg-white z-[45] md:hidden transition-transform duration-400 ease-out shadow-2xl ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}
            >
                <div className="flex flex-col h-full overflow-y-auto p-8 pt-20">
                    <nav className="space-y-1">
                        <Link to="/" className="block px-4 py-3 text-lg font-semibold text-primary hover:bg-gray-50 rounded-xl transition-colors">Home</Link>
                        <Link to="/projects" className="block px-4 py-3 text-lg font-semibold text-primary hover:bg-gray-50 rounded-xl transition-colors flex items-center gap-2">
                            Projects <span className="bg-accent-new text-[10px] text-white px-1.5 py-0.5 rounded font-bold">New</span>
                        </Link>
                        <Link to="/blog" className="block px-4 py-3 text-lg font-semibold text-primary hover:bg-gray-50 rounded-xl transition-colors">Blog</Link>
                        <Link to="/about" className="block px-4 py-3 text-lg font-semibold text-primary hover:bg-gray-50 rounded-xl transition-colors">About us</Link>
                        <Link to="/contact" className="block px-4 py-3 text-lg font-semibold text-primary hover:bg-gray-50 rounded-xl transition-colors">Contact</Link>
                    </nav>

                    <div className="border-t border-gray-100 pt-6 mt-6">
                        <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-4 px-4">Pages</p>
                        <a href="#" className="block px-4 py-2.5 text-sm text-text-secondary hover:text-primary transition-colors flex items-center gap-2">
                            Career <span className="bg-red-500 text-[9px] text-white min-w-[18px] h-[18px] rounded-full flex items-center justify-center font-bold">4</span>
                        </a>
                        <a href="#" className="block px-4 py-2.5 text-sm text-text-secondary hover:text-primary transition-colors">Privacy Policy</a>
                        <a href="#" className="block px-4 py-2.5 text-sm text-text-secondary hover:text-primary transition-colors">Terms of Service</a>
                    </div>

                    <div className="mt-auto pt-8">
                        <Link
                            className="w-full bg-primary hover:bg-primary-hover text-white px-6 py-3.5 rounded-full text-[14px] font-medium transition-all flex items-center justify-center gap-2"
                            to="/contact"
                        >
                            Book A Call
